...
x_data_set <- read.csv('hw02_data_set_images.csv', header = FALSE)

#reading y_data into memory
y_data_set <- read.csv('hw02_data_set_labels.csv', header = FALSE)
...
sigmoid <- function(x, w, b) {
  ...
  return (1 / (1 + exp(-(x %*% w + broad_b))))
}

#gradient functions
gradient_w <- function(x_train, y_truth, y_predict){
  return (-t(t((y_truth - y_predict) * y_predict * (1 - y_predict)) %*% x_train))
}

gradient_b <- function(y_truth, y_predict){
  return (-colSums(((y_truth - y_predict) * y_predict * (1 - y_predict))))
}
...
#where weight is a 320*5 matrix and bias is a 1*5 matrix
...
error_values <- c()
while(TRUE) {
  y_train_predict <- sigmoid(x_train, w, b)
  
  error <- 0.5 * sum((y_train_truth - y_train_predict)**2)
...
#plotting error values over iterations
...
#testing the model
#calculating and printing the train confusion matrix
y_test_predict <- sigmoid(x_test, w, b)
