...
\begin{equation}
	\begin{split}
		...
		\mbox{with respect to}\;\;& \boldsymbol{w} \in \mathbb{R}^{D},\;\; \boldsymbol{\xi}^{+} \in \mathbb{R}^{N},\;\; \boldsymbol{\xi}^{-} \in \mathbb{R}^{N},\;\; b \in \mathbb{R} \\
...
```{r}
inputPanel(
  sliderInput("s", label = "log2(Kernel width parameter) log2(s):", min = -2, max = 5, step = 1, value = 3, animate = TRUE),
  sliderInput("C", label = "log10(Regularization parameter) log10(C):", min = -2, max = 5, step = 1, value = 3, animate = TRUE),
  sliderInput("tube", label = "Tube width tube:", min = 5, max = 25, step = 5, value = 10, animate = TRUE)
)
